...
public class FakeGenerator3 {
    ...
    public static List<List<Output1>> encodeOutputs(){
        ...
        //get all subjects
        List<Subject1> subject_list = Subjects_initializer.getSubjectsDoubled();
        ...
        //double periods
        List<Day_Double_Periods> doubleAblePeriods = null;
        
                  
             //loop through streams
             for(Stream s : streams_list){
                 
                 doubleAblePeriods = Periods_initializer.getDoubleableLessons();
                ...
                if( s.getId() == 2 && s.getClass_id() == 3){
                     ...
                     newList = Subjects_initializer.getSubjectsf3vDoubled();
                }
                ...
                for( Subject1 subject : newList ){
                       ...
                       //  System.out.println("Teacher----" + t);
                         ...
                         int doubleAllocated = 0;

                         do{

                             int try_counter = 0;
                             Output1 oput1 = null;
                             Output1 oput1double = null;
                             Output1 oput1double2  = null;
                             boolean not_allocated = true;
                             boolean not_allocated_double = true;
                             
//                             //Allocate doubles first
//                             //------------------------------------------DOUBLES ALLOCATION-------------------
                             if( subject.getNo_of_doubles() > 0 && doubleAllocated < subject.getNo_of_doubles() ){
                                 Output1 outPeriod1 = null;
                                 Output1 outPeriod2 = null;
                                 
                                 System.out.println("###################################################################");
                                 System.out.println("Allocating double");
                                 System.out.println("###################################################################");
                                 /**
                                  * We need to pick a random double period---check whether
                                  * ---theres another double for same lesson same day
                                  * ---if selected teachers are free that time
                                  * Blacklist the days selected to avoid a triple or quadrable
                                  * ---blacklist the involved lessons to avoid overwriting
                                  */
                                 
                                 //get a double period from double periods list
                                  //int index = new Random().nextInt(doubleAblePeriods.size());
                                  //Day_Double_Periods ddP = doubleAblePeriods.get(index);

                                  
                             while(not_allocated && try_counter > -1){ 

                                     int indexdouble = new Random().nextInt(doubleAblePeriods.size());
                                     ddP = doubleAblePeriods.get(indexdouble);

                                     oput1double = new Output1(ddP.getDayId(), ddP.getPeriod1Id(), t.getId(), subject.getId(), s.getId(), s.getClass_id());
                                     oput1double2 = new Output1(ddP.getDayId(), ddP.getPeriod2Id(), t.getId(), subject.getId(), s.getId(), s.getClass_id());
                                     //check that teacher is free as selected time

                                     if(output_list.contains(oput1double) || output_list.contains(oput1double2) || selecteddayIds.contains(ddP.getDayId()) || isTeacherBusy2(t, ddP, olist)){//teacher not free need to loop
                              ...
                              output_list.add(oput1double);
                              output_list.add(oput1double2);
                           ...
                           //  }
                             ...
                             if( ( subject.getId() == 18 || subject.getId() == 15 || subject.getId() == 16) && (s.getId() == 1 && s.getClass_id() == 3) ){
                                 f3VsameOutputList.add(oput1double); 
                                 f3VsameOutputList.add(oput1double2); 
                                System.out.println("===================================================================PPPPPP=======");
                        
                             }

                             writeLog(oput1double.toString(), filenames);
                             
                             writeLog(oput1double.toString(), filenames);
                             
                             day_period_list.add(dayPeriod);
                             selecteddayIds.add(ddP.getDayId());
                             doubleAblePeriods.remove(ddP);
                             added++;
                             added++;
     ...
     * @return 
     */
    public static boolean isTeacherBusy2(Teacher teacher , Day_Double_Periods ddP , List<List<Output1>> olist){
        
        for( List<Output1> outputList : olist ){
            
            for( Output1 output1 : outputList ){
                
                if( output1.getDayId() == ddP.getDayId() && (output1.getPeriodId() == ddP.getPeriod1Id() || output1.getPeriodId() == ddP.getPeriod2Id() ) && output1.getTeacherId() == teacher.getId() ){
